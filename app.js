...
// SIGN UP / LOGIN (Standard handlers)
...
app.post('/signup', async (req, res) => {
  ...
  try { await pool.query('INSERT INTO users (username, password) VALUES ($1, $2)', [username, hashed]); res.redirect('/login'); } 
...
app.post('/login', async (req, res) => {
  ...
  if (result.rows.length > 0 && await bcrypt.compare(password, result.rows[0].password)) { req.session.username = username; res.redirect('/forum'); } 
  else { res.send('Invalid. <a href="/login">Back</a>'); }
});
...
app.get('/forum', async (req, res) => {
    ...
    try {
        ...
        let topicListHtml = topics.map((t, i) => `<li style="margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center; background: rgba(0,0,0,0.65); padding: 5px 10px; border-radius: 4px;"><a href="/topic/${i + 1}" style="color: #4CAF50; text-decoration: none; font-weight: bold;">${i + 1}. ${t}</a> <span style="background: #333; color: #00ffff; font-size: 0.7em; padding: 2px 6px; border-radius: 8px;">${countMap[i + 1] || 0}</span></li>`).join('');
        res.send(`<body ${kittenBG}><div style="background: rgba(0,0,0,0.5); min-height: 100vh; padding: 20px 40px;"><div style="display: flex; justify-content: space-between; align-items: center;"><div><h1 style="color: #4CAF50; margin:0;">🌌 BSMeSomeMorePlease</h1><p style="color: #00ffff; margin:0;">Welcome, <b>${user}</b></p></div><img src="https://placecats.com/80/80?t=${Date.now()}" style="border: 2px solid #4CAF50; border-radius: 50%;"></div><hr style="border: 0.5px solid #333; margin: 20px 0;"><div style="display: grid; grid-template-columns: 1fr 1fr; gap: 5px 40px;"><ul style="list-style: none; padding: 0;">${topicListHtml}</ul></div></div></body>`);
    } catch (err) { res.send("Error."); }
});

// TOPIC PAGE (Fixed Guest Access)
app.get('/topic/:id', async (req, res) => {
            ...
            messagesHtml = result.rows.map(row => `
                <div style="border: 1px solid #30363d; padding: 10px; margin-bottom: 8px; background: rgba(22, 27, 34, 0.92); border-radius: 6px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #333; padding-bottom: 5px; margin-bottom: 5px;">
                        <div><h4 style="color: #00ffff; margin: 0; font-size: 0.9em; display:inline;">${row.heading || 'GENERAL'}</h4><span style="color: #888; font-size: 0.7em; margin-left: 10px;">by ${row.author}</span></div>
                        <div style="display: flex; align-items: center; gap: 10px;"><span style="color: #666; font-size: 0.65em;">${row.time}</span>
    ...
    const postSection = (user === "Guest") 
        ? `<div style="background: rgba(22, 27, 34, 0.9); padding: 15px; border-radius: 8px; border: 1px solid #4CAF50; max-width: 500px;"><p style="color: #4CAF50; margin: 0;">Guests can read, but you must <a href="/signup" style="color: #00ffff; font-weight: bold;">Sign up</a> or <a href="/login" style="color: #00ffff; font-weight: bold;">Login</a> to post!</p> <button onclick="window.location.href='/forum'" style="background: #333; color: white; border: none; padding: 8px 15px; border-radius: 4px; cursor: pointer; margin-top: 10px;">Back</button></div>`
        : `<form action="/post/${topicId}" method="POST" style="background: rgba(22, 27, 34, 0.9); padding: 15px; border-radius: 8px; border: 1px solid #00ffff; max-width: 500px;"><input type="text" name="heading" placeholder="Heading..." required style="width: 100%; padding: 8px; margin-bottom: 8px; background: #0d1117; color: white; border: 1px solid #333;"><textarea name="content" placeholder="Message..." required style="width: 100%; height: 60px; background: #0d1117; color: white; border: 1px solid #333;"></textarea><button type="submit" style="background: #238636; color: white; border: none; padding: 8px 15px; border-radius: 4px; cursor: pointer; margin-top: 10px;">Post as ${user}</button><button type="button" onclick="window.location.href='/forum'" style="background: #333; color: white; border: none; padding: 8px 15px; border-radius: 4px; cursor: pointer; margin-top: 10px; margin-left: 5px;">Back</button></form>`;

    res.send(`<body ${kittenBG}><div style="background: rgba(0,0,0,0.5); min-height: 100vh; padding: 20px 40px;"><h2 style="color: #4CAF50;">Topic #${topicId}</h2> <div style="margin-bottom: 20px;">${messagesHtml}</div> ${postSection} </div></body>`);
});
...
app.post('/delete-post/:postId', async (req, res) => {
    ...
    if (post.rows[0] && (post.rows[0].author === user || user === "Ghostrider")) { await pool.query('DELETE FROM posts WHERE id = $1', [req.params.postId]); }
