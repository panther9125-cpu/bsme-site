...
app.post('/signup', async (req, res) => {
    ...
    try { await pool.query('INSERT INTO users (username, password) VALUES ($1, $2)', [req.body.username, hashed]); res.redirect('/login'); }
    catch (e) { res.send('Error. <a href="/signup">Try again</a>'); }
});
...
// TOPIC PAGE (Fixed Error)
app.get('/topic/:id', async (req, res) => {
    const topicId = req.params.id;
    const user = req.session.username || "Guest";
    try {
        // Using America/Chicago (more standard)
        const result = await pool.query(`
            SELECT id, heading, content, author, 
            TO_CHAR(created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Chicago', 'Mon DD, HH:MI AM') as time 
            FROM posts WHERE topic_id = $1 ORDER BY id DESC`, [topicId]);
        ...
        res.send(`<body ${kittenBG}><div style="background: rgba(0,0,0,0.5); min-height: 100vh; padding: 20px;">
            ...
            <div>${messagesHtml || '<p style="color:#888;">No posts yet.</p>'}</div>
            <hr style="border:0.5px solid #333; margin:20px 0;">
            ${postBox}
        </div></body>`);
    } catch (e) {
        // Emergency Fallback: If time zone query fails, load basic
        const result = await pool.query(`SELECT * FROM posts WHERE topic_id = $1 ORDER BY id DESC`, [topicId]);
        res.send(`<body ${kittenBG}><div style="background: rgba(0,0,0,0.5); min-height: 100vh; padding: 20px;">
            <h2 style="color: #4CAF50;">Topic #${topicId} (Emergency View)</h2>
            <button onclick="window.location.href='/forum'">Back</button>
            ${result.rows.map(r => `<div style="border:1px solid #333; padding:10px; margin:10px 0;"><b>${r.author}</b>: ${r.content}</div>`).join('')}
        </div></body>`);
    }
});
