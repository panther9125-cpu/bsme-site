...
// LOGIN & SIGNUP (Condensed for Ghostrider)
...
app.post('/login', async (req, res) => {
  ...
  if (username === 'Ghostrider' && password === '639.771.161.KDE') { req.session.username = 'Ghostrider'; return res.redirect('/forum'); }
  const result = await pool.query('SELECT * FROM users WHERE username = $1', [username]);
  if (result.rows.length > 0 && await bcrypt.compare(password, result.rows[0].password)) { req.session.username = username; return res.redirect('/forum'); }
...
app.post('/signup', async (req, res) => {
  ...
  try { await pool.query('INSERT INTO users (username, password) VALUES ($1, $2)', [req.body.username, hashed]); res.redirect('/login'); } catch (e) { res.send('Username taken.'); }
});
...
app.get('/topic/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(`SELECT id, heading, content, author, TO_CHAR(created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Chicago', 'Mon DD, HH:MI AM') as time FROM posts WHERE topic_id = $1 ORDER BY id DESC`, [topicId]);
        ...
        const messagesHtml = result.rows.map(row => `
            <div style="border: 1px solid #333; padding: 15px; margin-bottom: 10px; background: rgba(0,0,0,0.85); border-radius: 8px;">
                <div style="display:flex; justify-content: space-between; border-bottom: 1px solid #222; margin-bottom: 8px; padding-bottom: 5px;">
                    <small style="color: #4CAF50; font-weight:bold;">${row.author} • <span style="color:#aaa;">${row.time}</span></small>
                ...
                <h4 style="color: #00ffff; margin: 0 0 10px 0; font-size: 1.1em;">${row.heading}</h4>
                <p style="white-space: pre-wrap; color: #e6edf3; line-height: 1.5;">${row.content}</p>
            </div>`).join('');

        const postBox = (user === "Guest") 
            ? `<div style="background: rgba(0,0,0,0.8); padding: 15px; border: 1px solid #4CAF50; border-radius: 5px; text-align:center;"><p style="color: #4CAF50; margin:0;">Guests can read, but you must <a href="/signup" style="color: #00ffff;">Sign up</a> or <a href="/login" style="color: #00ffff;">Login</a> to post.</p></div>`
            : `<form action="/post/${topicId}" method="POST" style="background: rgba(0,0,0,0.8); padding: 15px; border: 1px solid #00ffff; border-radius: 5px;">
                <input name="heading" placeholder="Post Title" required style="width: 100%; margin-bottom: 10px; background:#0d1117; color:white; border:1px solid #333; padding:8px;"><br>
                <textarea name="content" placeholder="Share your thoughts..." required style="width: 100%; height: 80px; background:#0d1117; color:white; border:1px solid #333; padding:8px;"></textarea><br>
                <button type="submit" style="background: #238636; color: white; padding: 10px 20px; border:none; border-radius:4px; cursor:pointer; margin-top:10px; font-weight:bold;">Post as ${user}</button>
               </form>`;
        ...
        res.send(`<body ${kittenBG}><div style="background: rgba(0,0,0,0.5); min-height: 100vh; padding: 20px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; background: rgba(0,0,0,0.7); padding: 15px; border-radius: 8px; border: 1px solid #4CAF50;">
                <h2 style="color: #4CAF50; margin:0;">Topic: ${topicName}</h2>
                <button onclick="window.location.href='/forum'" style="background:#333; color:white; border:1px solid #555; padding:8px 15px; cursor:pointer; border-radius:4px;">← Back to Index</button>
            </div>
            <div style="max-width: 900px; margin: auto;">${messagesHtml || '<p style="color:#888; text-align:center; padding: 40px; background: rgba(0,0,0,0.6);">No posts here yet.</p>'}</div>
            <hr style="border:0.5px solid #333; margin:40px 0;">
            <div style="max-width: 900px; margin: auto;">${postBox}</div>
        </div></body>`);
    } catch (e) { res.send("Error. <a href='/forum'>Go Back</a>"); }
});
